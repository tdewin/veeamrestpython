			...
			if response.status_code < 400 and idheader in response.headers :
				...
				for link in root.iter("{{{ns}}}{tag}".format(ns=xmlnamespace,tag="Link")):
					if "Href" in link.attrib and "Rel" in link.attrib:
						...
						elif "Type" in link.attrib and link.attrib["Type"] == "JobList":
							hrefjoblist = link.attrib["Href"]
				
				try:
					if hrefjoblist:
						#entity presentation (not reference) https://helpcenter.veeam.com/docs/backup/rest/get_jobs_id.html?ver=95
						print("Found job list link")
						response = requests.get(hrefjoblist,headers=headers,verify=verifyssl)
						if response.status_code < 400:
							root = elementtree.fromstring(response.text)
							for job in root.iter("{{{ns}}}{tag}".format(ns=xmlnamespace,tag="Job")):
								jobtype = job.findtext("{{{ns}}}{tag}".format(ns=xmlnamespace,tag="JobType"))
								print("Job: {0:30} {1:10} {2}".format(job.get("Name"),jobtype,job.get("UID")))
						else:
