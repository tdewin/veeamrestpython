			...
			if response.status_code < 400 and idheader in response.headers :
				...
				for link in root.iter("{{{ns}}}{tag}".format(ns=xmlnamespace,tag="Link")):
					if "Href" in link.attrib and "Rel" in link.attrib:
						...
						elif "Type" in link.attrib and link.attrib["Type"] == "QueryService":
				...
				try:
					if hrefqueryservice:
						...
						#the service itself ends on Svc <= https://helpcenter.veeam.com/docs/backup/rest/get_loginsessions.html?ver=95
						#however a query is without the Svc
						#querying https://helpcenter.veeam.com/docs/backup/rest/query_and_or.html?ver=95
						querylink = hrefqueryservice.replace("querySvc","query")
						
						jobname = "Backup Job 1"
						params={'type':'Job','format':'Entities','filter':'name=="{0}"'.format(jobname)}
						response = requests.get(querylink,headers=headers,verify=verifyssl,params=params)
						
						if response.status_code < 400:
							root = elementtree.fromstring(response.text)
							jobquery = root.findall(".//{{{ns}}}{tag}[@Name='{name}']".format(ns=xmlnamespace,tag="Job",name=jobname))
							if len(jobquery) > 0:
								job = jobquery[0]
								startlinkquery = job.findall(".//{{{ns}}}{tag}[@Rel='{rel}']".format(ns=xmlnamespace,tag="Link",rel="Start"))
